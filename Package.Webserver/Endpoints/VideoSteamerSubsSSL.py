from flask import Flask, send_file, render_template_string, Response, send_from_directory,render_template,request
...
def partial_response(path, start, end=None):
    """Handle partial content response for video streaming"""
...
def handle_file_request(directory, filename, content_type=None):
    """Generic file handler with improved error handling"""
    try:
        ...
        if ".mp4" in path:
            ...
            if range_header:
                ...
                return partial_response(path, byte_start, byte_end)

        # Regular file serving with error handling
        try:
        ...
        logging.error(f"Unexpected error handling {filename}: {str(e)}")
...
def stream_video(filename):
    ...
    return handle_file_request(path, file)
...
if __name__ == '__main__':
    app.run(
        ...
        port=5000, 
        debug=True,
        ssl_context=('../../Package.Certificates/cert.pem','../../Package.Certificates/key.pem'),
        threaded=True)
